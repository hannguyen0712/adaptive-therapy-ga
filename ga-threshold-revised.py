...
class Genome:
    ...
    Fixed-length genome with exactly 2 genes:
      ...
      gene[1] = cell_threshold ∈ [1.0, 1.5] — multiplier on W₀ (warmup cell count)
...
def random_genome(cfg):
    return Genome(
        ...
        cell_threshold=round(random.uniform(1.0, 1.5), 3),
    )
...
def drug_func(timestep, drug_idx, num_cells, num_resistant):
    global _cumulative_dose, _interval_max_dose, _just_treated
    ...
    # At snapshot: log the interval, then decide whether to treat
    if timestep % SAMPLE_PERIOD == 0:
        if timestep > 0:
...
def preflight(cfg):
    ...
    print(f"  Threshold gene: [1.0, 1.5] × W₀ = [{warmup_cells:,}, {2*warmup_cells:,}]")
...
def _eval_one(args):
    ...
    samples_dir = os.path.join(cfg["SAMPLES_DIR"], f"genome_{i}")
    traj = run_simulation(path, samples_dir, cfg)
    fit = compute_fitness(traj, cfg)
    # clean up after fitness is computed
    if os.path.exists(samples_dir):
        shutil.rmtree(samples_dir)
        ...
        with ProcessPoolExecutor(max_workers=cfg["MAX_WORKERS"]) as ex:
            ...
            for fut in as_completed(futs):
                ...
                print(f"  {i:02d}: fit={f:>3}  "
                      ...
                      f"[d={population[i].dosage:.2f} t={population[i].cell_threshold:.3f}×W₀]",
                      flush=True)
    else:
        for a in args:
            i, f, t = _eval_one(a)
...
def mutate(genome, cfg):
    ...
    Gaussian perturbation:
      ...
      cell_threshold: N(0, 0.1) clamped to [1.0, 1.5]
    """
    ...
    # Gene 1: cell_threshold — multiplier on W₀, clamped to [1.0, 1.5]
    if random.random() < mr:
        genome.cell_threshold = round(max(1.0, min(1.5, genome.cell_threshold + random.gauss(0, 0.1))), 3)
def genetic_algorithm(cfg):
    ...
    for gen in range(cfg["GENERATIONS"]):
        ...
        # Among all genomes tied at max fitness, pick lowest dose
        max_fit_indices = [j for j, f in enumerate(fit) if f == gb]
        gi = min(max_fit_indices, key=lambda j: trajs[j].cumulative_dose if trajs[j] else float("inf"))
    ...
    if history:
        ...
        if best_traj and best_traj.timesteps:
            ...
            if best_traj.dose_per_step:
                warmup_idx = best_traj._warmup_idx
                pw_timesteps = best_traj.timesteps[warmup_idx:]
                pw_counts = best_traj.cell_counts[warmup_idx:]
                n_pts = min(len(pw_timesteps), len(best_traj.dose_per_step))
                on_t, on_c, off_t, off_c = [], [], [], []
                for j in range(n_pts):
                    if best_traj.dose_per_step[j] > 0:
                        on_t.append(pw_timesteps[j])
                        on_c.append(pw_counts[j])
                    else:
                        off_t.append(pw_timesteps[j])
                        off_c.append(pw_counts[j])
